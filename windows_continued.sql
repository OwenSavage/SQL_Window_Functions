...
WITH temp AS (
    SELECT NAME,
           ...
           SPORT,
           EARNINGS,
           ROW_NUMBER() OVER (PARTITION BY YEAR ORDER BY EARNINGS ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS YEARLY_RANK
    FROM TOP_ATHLETES
)

SELECT * FROM temp WHERE YEARLY_RANK = 1 ORDER BY YEAR;
...
WITH TEMP AS (
    SELECT NAME,
           ...
           YEAR,
           SUM(EARNINGS)
           OVER (PARTITION BY NAME, SPORT ORDER BY YEAR ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW ) AS RUNNING_SUM
    FROM TOP_ATHLETES
),
...
TEMP_2 AS (
    SELECT
        ...
        EARNINGS,
        YEAR,
        RANK() OVER (PARTITION BY YEAR ORDER BY RUNNING_SUM DESC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS RUNNING_SUM_RANK
    FROM
        TEMP
    )

SELECT * FROM TEMP_2 WHERE RUNNING_SUM_RANK =1 ORDER BY YEAR;
